...
using MusicManagementApp.Models;
using System;

namespace MusicManagementApp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Jerry.FavoriteSongs.AddRange(Songs.Where(s => s.Title.StartsWith("B")));
            Maria.FavoriteSongs.AddRange(Songs.Where(s => s.Length.TotalSeconds > 360));
            Jane.FavoriteSongs.AddRange(Songs.Where(s => s.Genre == Genre.Rock));
            Stefan.FavoriteSongs.AddRange(Songs.Where(s => s.Genre == Genre.Techno && s.Length.TotalSeconds < 180));

            List<Person> personsWithFourOrMoreSongs = FansArray.Where(p => p.FavoriteSongs.Count >= 4).ToList();

            foreach (Person fan in personsWithFourOrMoreSongs)
            {
                Console.WriteLine($"{fan.FirstName} {fan.LastName} has {fan.FavoriteSongs.Count} favorite songs.");
            }
        }
    }
}
